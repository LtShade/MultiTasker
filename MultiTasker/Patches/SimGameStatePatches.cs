        ...
        static void Prefix(SimGameState __instance, bool passDay)
        {
           ...
           Dictionary<int,float> efficiencies = EfficienciesHelper.GetEfficiencies(__instance);
            ...
            for (int i = 1; i < j; i++)
            {
                ...
                {
                    //This is dead
                    __instance.MechLabQueue[i].PayCost(Convert.ToInt32((float)eff * 0.5 * __instance.MechTechSkill));
                }
                else
                {
                    __instance.MechLabQueue[i].PayCost(Convert.ToInt32((float)eff * __instance.MechTechSkill));
                }
                Mod.Log.Debug.Log($"Queue Position {i} Paid Cost: {Convert.ToInt32((float)eff * __instance.MechTechSkill)}");
            }
            if(efficiencies.Count == 0)
            {
                __instance.MechLabQueue[0].PayCost(Convert.ToInt32(1 * (float)__instance.MechTechSkill));
                Mod.Log.Debug.Log($"||Fallback|| Queue Postion 0 Paid Cost: {Convert.ToInt32(1 * (float)__instance.MechTechSkill)}");
            }
